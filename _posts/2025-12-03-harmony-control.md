...
가능하다. 가장 흔한 형태는 아래처럼 **목표각–현재각 오차**를 PID로 처리하는 방식이다.
...
`command_and_report.cpp`는 로봇에 대략 이런 형태로 보내고 있다.
...
- 바깥 루프가 보낸 `offset + stiffness`를 받아서
- 실제 모터를 움직인다  
...
3. **바깥/안쪽 루프가 서로 간섭**해서 싸우며 흔들림이 생김 (이중 루프 충돌)
